#!/bin/zsh
# One-shot scheduled submit of Block 14 calibrated quick-dirty notebook output
# after the Block 13 raw quick-dirty submit window. Idempotent: skips if already
# submitted or if any completed public score is already below target.
set -u
...
KERNEL="ucheozoemena/umud-submission-quickdirty-cal"
VERSION="1"
FILE="submission.csv"
MESSAGE="block14-quickdirty-cal"
TARGET="0.6"
PLIST="$HOME/Library/LaunchAgents/com.uche.umud-block14-submit.plist"
LOG="$REPO/data/kaggle-outputs/block14-quickdirty-cal-scheduled.log"

cd "$REPO" || exit 1
mkdir -p "$(dirname "$LOG")"
echo "=== $(date -u '+%Y-%m-%d %H:%M:%S') UTC scheduled block14 submit ===" >> "$LOG"
...
if [ -n "$best_score" ]; then
  echo "Best completed public score before block14: $best_score" >> "$LOG"
  if awk "BEGIN { exit !($best_score < $TARGET) }"; then
    echo "Target already met (< $TARGET); skipping block14 submit." >> "$LOG"
