"""Generate notebooks/train-apo-mounted/train-apo-mounted-phase-3.ipynb — train apo from prep dataset."""
import json
...
cells.append(
    md(
        """# UMUD — Train Apo U-Net from Prep Dataset

**GPU notebook** — mounts an **apo prep** dataset (`dataset_sources`). No competition TIFF scan or inline align.
...
RANDOM_SEED = 42
TRAIN_RUN = 2  # 1=timing-50, 2=timing-200, 3=timing-524 (50% apo)
...
TRAIN_PROFILES = {
    1: {
        "dataset_slug": "ucheozoemena/umud-aligned-apo-timing-50",
        "epochs": 1,
        "label": "AT1 apo 50×1ep",
    },
    2: {
        "dataset_slug": "ucheozoemena/umud-aligned-apo-timing-200",
        "epochs": 1,
        "label": "AT2 apo 200×1ep",
    },
    3: {
...
learn.export(WORKING / "apo_baseline.pkl")
...
def main() -> None:
    out = Path(__file__).resolve().parents[1] / "notebooks/train-apo-mounted"
    write_nb(out / "train-apo-mounted-phase-3.ipynb")
