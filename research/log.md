...
- **What worked:** (1) per-connected-component PCA fascicle angle (the big unlock, block25: 0.926->0.905); (2) apo-mask polarity fix (block28/29: 0.905->0.897). Both are real, durable findings.
- **Exhausted levers (all flat or negative on the per-image signal r~0.29 PA / 0.40 MT/FL):** PA-center calibration (vertex ~16deg, already optimal); ensemble weight tuning; geometry variants (perp-line MT, structure-tensor angle, skeletonize); resolution 512->640; epochs 18->32; dilated fascicle masks (dice 0.21->0.65 but PA worse 0.20 — dilation merges fascicles). See block24-30 sections.
- **Why <0.6 is unreached:** constant-prediction floor ~0.92; reaching <0.6 needs ~35% MAE cut => per-image r~0.76 across targets, vs our ~0.3-0.4. That is a model-quality gap, not tuning.

**The documented path to <0.6 (for a future push):** DL_Track_US (PaulRitsche/DL_Track_US) — the domain tool (TF/Keras VGG16-UNet + fascicle-tracking geometry), PA within 1.5deg of manual, default benchmark already 0.679; leaders fine-tune it to 0.33-0.5. Plan: locate/download its OSF weights -> host as a Kaggle dataset -> hidden-safe TF inference notebook replicating its fascicle-tracking geometry -> calibrate/ensemble/fine-tune on the competition masks toward <0.6. Large, multi-session, uncertain. Trained SMP weights are saved (block20/28 lakhindar+corrected, block30 dilated) if revisited.
...
**Block29 scored 0.89723 — NEW BEST** (from block25 0.90470), the MT/FL gain from the apo
polarity fix landing on the LB. Confirms each lever buys ~0.01 and the SMP plateau is real.

**Geometry refinement also exhausted (2026-06-27):** tested better MT (perpendicular
line-fit between fitted aponeurosis lines) = 0.406 vs median-row 0.400 (no gain); better
...
Result before scoring: kernel v1 **COMPLETE**. Downloaded output at `data/kaggle-outputs/block23-blend-block19-smpcal/` has **309 rows / 0 NaN / 0 duplicate IDs**. Final medians PA/FL/MT are **17.45° / 74.36 mm / 20.50 mm** with means **17.52° / 73.61 mm / 20.36 mm**. The final CSV matches the intended formula `0.65*Block19 + 0.35*SMP-cal` to floating tolerance (max deltas <1.5e-14). Compared with Block19, the blend changes PA/FL/MT by mean absolute **0.97° / 2.90 mm / 0.33 mm**; correlations remain **0.866 / 0.939 / 0.953**. SMP status is **304/309** `mt_ok+pa_ok` and **5/309** fallback. Output is structurally sane and hidden-safe because it recomputes predictions from mounted images and train-derived weights.

Score: notebook-output submit `block23-block19-smpcal-blend035` scored **0.95631** on 2026-06-25. This regressed vs Block19 **0.92609**. The calibrated SMP component looked plausible by range but moved public score the wrong way; do not continue simple Block19/SMP-cal blend weights without new evidence. If SMP is revisited, inspect a different signal extraction/calibration route first rather than just lowering the blend weight.
...
- Reset submit script: `scripts/submit_block19_scheduled.sh` submitted notebook output only (`-k ucheozoemena/umud-submission-blend-qdc-cxs5 -v 1 -f submission.csv -m block19-qdc-cxs5-refresh`) at **2026-06-25 00:26 UTC / 01:26 WAT**. Public score: **0.92609**. This is the best hidden-safe notebook result so far, but still above the **0.6** target.
...
Hidden-safe notebook: `notebooks/submission-seq-smooth/` computes cxs8 and quickdirty from mounted images, blends as Block 15, then writes `submission.csv = rolling-5 mean` by numeric image order. It also writes unsmoothed blend and rolling-5 median debug CSVs. Kaggle kernel `umud-submission-sequence-smooth` v1 **COMPLETE**; downloaded output at `data/kaggle-outputs/block18-sequence-smooth/` has **309 rows / 0 NaN** and exactly matches local rolling-5 mean of Block 15 (max deltas <1.5e-14).

Reset submit script: `scripts/submit_block18_scheduled.sh` submitted notebook output only (`-k ucheozoemena/umud-submission-sequence-smooth -v 1 -f submission.csv -m block18-seq-smooth5-mean`) at **2026-06-25 00:10 UTC / 01:10 WAT**. Public score: **0.96804**. Sequence smoothing regressed vs Block 15 and should not be repeated without a better sequence-grouping signal.
...
**Rationale:** Raw quick-dirty has the first serious per-image PA/FL path, but its centers are far from the current leaderboard-fit target: raw median PA/FL/MT **11.31° / 118.9 mm / 24.37 mm** vs cxs-s2 **18° / 74.5 mm / 21.6 mm** and fit centers near **17.15° / 76.9 mm / 19.76 mm**. The fixed transform is:
